...
<?php include 'elements/html_navigation.php'?>
    ...
    <thead>
        <tr>
            <?php foreach (MContainer::$DTFields as $key => $value){
                echo '<th>' . $value . '</th>';
            }?>
...
$(document).ready(function() {
    var table = $('#dtTable').DataTable( {
        ...
        "ajax": "js/datatables/scripts/container_list.php",
        "columnDefs": [ {
            "targets": -1,
            "data": null,
            "defaultContent": "<button>Details</button>"
        } ,{
            //false to hide ID field 
            "targets": [0],
            "visible": true,
            "searchable": true
    ...
    $('#dtTable tbody').on( 'click', 'button', function () {
        ...
        window.location.href = ("container_details?cmd=details&val_a="+ data[0] + "&val_b=" + data[2]);
    } );
